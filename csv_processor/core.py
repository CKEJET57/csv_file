...
def read_csv(
        file_path: str
) -> tuple[list[dict[Any, float | Any]], Sequence[str]]:
...
def apply_filter(
    ...
    for row in data:
        ...
        try:
            row_num = float(row_value) if not isinstance(
                row_value, (int, float)
            ) else row_value
            val_num = float(value)
...
def apply_aggregation(
    ...
    if operation not in AGGREGATIONS:
        raise ValueError(
            f"Неизвестная операция агрегации: {operation}."
            f" Доступные: {', '.join(AGGREGATIONS.keys())}"
        )
    
    try:
        values = [float(row[column]) if not isinstance(
...
def apply_order_by(
    ...
    if '=' not in condition:
        raise ValueError(
            "Условие сортировки должно быть в формате 'column=order'"
        )
    ...
    if order not in SORT_ORDERS:
        raise ValueError(
            f"Неизвестный порядок сортировки: {order}."
            f" Используйте 'asc' или 'desc'"
        )
    ...
    try:
        ...
        return sorted(
            data, key=lambda x: SORT_ORDERS[order](str(x.get(column, '')))
        )
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Обработчик CSV файлов с '
            'фильтрацией, агрегацией и сортировкой'
        ),
        epilog='Примеры:\n'
    ...
    parser.add_argument(
        '--aggregate', help='Агрегация (например, "rating=avg")'
    )
    parser.add_argument(
        '--order-by', help='Сортировка (например, "name=desc")'
    )
    ...
    try:
        ...
        if args.aggregate:
            ...
            print(
                tabulate(
                    result, headers="keys", tablefmt="grid", floatfmt=".2f"
                )
            )
        else:
            print(
                tabulate(data, headers="keys", tablefmt="grid", floatfmt=".2f")
            )
